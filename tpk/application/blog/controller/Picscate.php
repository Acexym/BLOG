...
class Picscate extends Base
{
    ...
    public function add(){
       if(request()->isPost()){
           ...
           $data['uid']=$res['uid'];     
           //方法3：修改配置项 ，是否严格检查字段是否存在 'fields_strict'   => false,
    ...
    public function lst(){
        ...
        $modelPicscate = model('Picscate');
        // 查询数据集
        $res=$modelPicscate->where('uid',$res['uid'])->order('sort', 'desc')
            ->paginate(5,false);
        //分配单个变量到模板
...
//                $this->success("更新成功",'lst');
    ...
    public function del($id){
        ...
        $modelblogcat=model('Picscate');
        $result=$modelblogcat->where('cid',$id)->delete();
        if($result==1){
            return json(['code'=>1,"msg"=>'删除成功']);
            //$this->success("删除成功",'blog/Picscate/lst','',2);
        }else{
