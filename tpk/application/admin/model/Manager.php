...
//  
        ...
        $uid=$result->id;
        $datas=['uid'=>$uid,'group_id'=>$a];
//        判断中间表中是否存在记录
        $res=db("auth_group_access")->where("uid",$uid)->find();
        if($res){
            db("auth_group_access")->where('uid',$uid)->update(['group_id'=>$a]);
        }else{
    ...
    public static function login($data){
        ...
        if(!$res){
            return returnjson(0,"账号不存在");
				...
				$status=self::where('account',$data['account'])->value('status'); 
//				dump($status);die;
				if($status==0){
					  return returnjson(0,"账号被停用");
				}
